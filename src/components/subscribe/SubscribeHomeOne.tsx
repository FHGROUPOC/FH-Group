...
const SubscribeHomeOne = () => {
  ...
  const handleSuubmit = async(e : any) => {
    e.preventDefault();

    if(e.target.email.value){
      const postit = await fetch ('/api/subscribers',{
        method:"POST",
        headers:{
      ...
      <div className="container">
        <div className="cs_newsletter cs_style_1 cs_primary_bg cs_shape_wrap_1 cs_parallax">
          ...
          <form className="cs_newsletter_form" onSubmit={handleSuubmit}>
            <input required type="email" name='email' className="cs_newsletter_input" placeholder="Enter Your Email" />
